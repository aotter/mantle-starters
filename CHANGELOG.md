...
The repository version reflects the `@aotter/create-mantle` scaffolder published to npm at each tag (and mirrored as a tarball asset on the corresponding GitHub release). Individual starter packages pin `@aotter/mantle-*` versions independently inside their own `package.json`.
...
- `packages/create-mantle/` scaffolder, published to npm as `@aotter/create-mantle` and also attached as a tarball asset on each GitHub release.
