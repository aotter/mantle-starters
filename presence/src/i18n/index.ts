 ...
 * Schema-stored content (page bodies, page titles) stays in the
 * runtime; this is starter chrome (nav, headings, error copy, form
 * labels) that doesn't fit a CMS-row shape.
 *
 ...
 *      `I18nBundle`).
 *   2. Import + register in `I18N_BUNDLES` below.
 *   3. Add `"ja"` to `siteDefaults.locales` in `src/mantleConfig.ts`.
 *   4. Add `page-translations` entries for that locale via fixture or
 *      admin.
 *
