...
// declared HTTP triggers, Views, /favicon.svg) keeps rendering even
// when BETTER_AUTH_SECRET is missing — public previews shouldn't fail
// closed because the operator hasn't wired admin yet.
...
function noOpAuth(): Auth {
  if (!warnedAuthMissing) {
    ...
    console.warn(
      "[blank] BETTER_AUTH_SECRET unset — serving public routes only; /admin /mcp /api/auth will return 503.",
    );
  }
  return {
